...
import { myPagDeliveryStatusBoxData, myPageOrderStatusBoxData } from "@/libs/myPageData";
...
const MyOrderSummary = () => {
    return (
        <div className="px-[16px]">
                ...
                <ul className="flex justify-around">
                    {myPagDeliveryStatusBoxData.map((item: LinkType, index) => (
                        <li key={item.id} className="flex">
                            <Link className="felx items-center justify-center mr-[5px]" href={item.url ? item.url : "/"}>
                                ...
                                <div className="leading-4 text-center">
                                    <span className="text-[12px] text-[#444444] ">{item.name}</span>
                                </div>
                            </Link>
            ...
            {/* 주문/배송조회 바로가기 */}
            <Link href={"/mypage/orderInfo"} className="h-[45px] flex items-center justify-center text-[13px] text-[#444444]">
                주문/배송 조회 보러가기
                <RightHalfTriangle />
            </Link>
        </div>
