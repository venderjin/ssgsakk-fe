            ...
            <div className="mt-[18px]">
                <ul className="flex flex-wrap h-[310px] ">
                    {myPageQuickMenuData.map((item: LinkType) => (
                        <li key={item.id} className="basis-1/5  flex items-center justify-center text-center">
                            <Link href={item.url ? item.url : "/"}>
                                <Image src={item.icon || "/"} alt={item.name} width={65} height={65} />

                                <div className="max-h-[20px] tracking-tight text-center leading-5">
                                    <span className="text-[12px] text-[#666666] text-tight">{item.name}</span>
                                </div>
                            </Link>
                        </li>
                    ))}
