...
const ManageShippingList = ({ shippingData }: { shippingData: ShippingInfoType[] }) => {
                ...
                <ul>
                    {shippingData &&
                        shippingData.map((item: ShippingInfoType) => (
                            <li key={item.shippingAddressSeq} className="relative block py-[20px] pl-[40px] border-b border-b-[#f1f1f1] h-min-[19px]">
                                <ShippingInfoBox shippingData={item} setCheckedAddressId={setCheckedAddressId} checkedAddressId={checkedAddressId} />
                            </li>
                        ))}
                </ul>
            </section>
