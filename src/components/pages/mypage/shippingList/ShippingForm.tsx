...
const schema = yup.object().shape({
    ...
    receiverPhone: yup.string().required("휴대폰 번호를 입력해주세요").matches(regex.phone, "'-' 제외 11자리를 입력해주세요"),
...
const ShippingForm = () => {
    ...
    const shippingAddressId = params.get("shippingAddressId") ? Number(params.get("shippingAddressId")) : null;
    ...
    return (
        <div>
            <BackArrowHeader title={shippingAddressId ? "배송지 수정" : "배송지 추가"} />
            {openAddress ? (
