                    ...
                    {homeMainAdvertisementCarouselData.map((ad: HomeMainAdvertisementCarouselType, index: number) => (
                        <SwiperSlide key={ad.id}>
                            <div className="h-[100%] relative">
                                <Image src={ad.imgPath} alt="swiper_ad" fill priority={index === 0 ? true : false} />
                                <div className="absolute inset-x-0 bottom-[35px] z-102 bg-transparent">
                                    {ad.title.split("\n").map((line, index) => (
                                        <p className="font-Pretendard text-white text-[25px] font-bold text-center" key={index}>
                                            {line}
                                        </p>
                                    ))}
            ...
            {adModalIsOpen && (
                <Modal>
                    <div className="sticky top-0 bg-white w-full h-[48px] py-[12px]">
                        <p className="font-Pretendard text-[16px] font-semibold text-center tracking-wide">전체 보기</p>
                        <div className="w-[50px] h-[24px] my-[12px]flex absolute right-0 top-0 bottom-0 items-center justify-center">
                            ...
                            >
                                <Image src="/images/etc/closeIcon.png" alt="closeIcon" width={28} height={28} priority={true} />
                            </button>
                        </div>
                    </div>
                    <div className="h-full mt-3">
                        {homeMainAdvertisementCarouselData.map((ad: HomeMainAdvertisementCarouselType, index: number) => (
                            <div key={ad.id} className=" h-[160px] mx-3 my-[20px] border-b-[1px] flex flex-row gap-4">
                                <Image src={ad.imgPath} alt="ad_main" width={120} height={140} priority={true} style={{ width: "120px", height: "140px" }} />
                                <div className="flex flex-col justify-center item-center">
                                    {ad.title.split("\n").map((line, index) => (
                                        <p className="font-Pretendard text-black text-[18px] font-bold" key={index}>
                                            {line}
                                        </p>
                                    ))}
                                    <p className="font-Pretendard text-black text-[13px] ">{ad.subtitle}</p>
                                </div>
                            </div>
                        ))}
                    </div>
