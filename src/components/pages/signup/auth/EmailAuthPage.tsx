...
const EmailAuthPage = () => {
    ...
    const onChangeAgreementList = useCallback((data: CheckBoxType) => {
        setCheckBoxes({ ...data });
    }, []);
    ...
    const sendCodeHandler = () => {
        if (!checkBoxes.checkAll) return alert("개인정보 이용 및 제공 동의에 동의해주세요.");

        if (!emailRegex.test(userEmail)) return alert("올바른 이메일 형식이 아닙니다.");
        ...
        <div className=" bg-[#f0f0f0] px-[15px] pb-[20px]">
            ...
            <section className="h-[100px] mt-[20px]">
                <form className="bg-[#fff]" action={sendCodeHandler}>
                    ...
                    {!isOpen && (
                        <button type="submit" className="base-btn bg-[#ff5452] text-white font-medium">
                            인증코드 발송
                        </button>
                    )}
                </form>
                    ...
                    <>
                        <div className="flex flex-col h-[72.5px] pt-[13px] pb-[12px] px-[15px] border-t border-t-[#f0f0f0] bg-[#fff]">
                            <div>
                                ...
                                <span onClick={sendCodeHandler} className="float-right text-[14px] text-[#222] underline cursor-pointer">
                                    재전송
                                </span>
                            </div>
                            <span className="pt-[5px] text-[11px] text-[#ff6967] ">
                                {remainingTime === 0 ? "인증번호 유효시간이 초과되었습니다." : `남은시간: ${timeFormatter(remainingTime)}`}
                            </span>
                        </div>

                        <button onClick={confirmHandler} className="base-btn bg-[#ff5452] text-white font-medium">
                            확인
                        </button>
                    </>
                )}
            ...
            <section className=" h-[74px] mt-[90px] font-Pretendard">
                <ul className="h-full text-[12px] text-[#222]">
                    <li className="relative pl-[7px] ">
                        <span className="absolute h-[4px] w-[4px] left-0 top-[3px] pr-[3px] text-[red]">*</span>
