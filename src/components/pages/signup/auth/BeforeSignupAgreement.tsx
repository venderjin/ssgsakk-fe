...
const BeforeSignupAgreement = ({ onChangeAgreement }: { onChangeAgreement: (agree: boolean) => void }) => {
    ...
    const checkAll = (updateCheckboxes: CheckBoxType) => {
        const { terms01, terms02, terms03, terms04, terms05, terms06 } = updateCheckboxes;
        ...
        <div className="px-[20px] pt-[15px]">
            <div className="flex  items-center mb-[10px] ">
                <input
                    ...
                    title="checkAll"
                    className={`
                ${checkBoxes.checkAll ? "bg-[position:0px_-300px]" : "bg-[position:0px_-200px]"}
                appearance-none mr-[10px] w-[17px] h-[17px] bg-no-repeat bg-check-icon  bg-[length:17px_400px]`}
                />
                <label className="font-bold w-[80%]  text-[12px] text-[#222] mr-[20px]" htmlFor="checkAll">
                    약관 전체 동의
                </label>
            </div>
            <div className="pl-[10px]">
                {beforesignupAgreementData.map((bigTerm: BeforeSignupAgreementType, index) => (
                    <div className="pb-[10px] [mt-[10px] border-t border-t-[#f0f0f0]" key={index}>
                                    ...
                                    <div className="flex content-center items-center">
                                        <div>
                                            <input
                                                ...
                                                onChange={onChangeCheckbox}
                                                className={`${checkBoxes[term.type] ? "bg-[position:0px_-300px]" : "bg-[position:0px_-200px]"}
                            appearance-none mr-[10px] w-[17px] h-[17px] bg-no-repeat bg-check-icon  bg-[length:17px_400px]`}
                                            />
                                        </div>
                                        <label className="w-[80%]  text-[12px] text-[#222] mr-[20px]" htmlFor={term.type}>
                                            {term.title}
                                        </label>
                                        <button
                                            onClick={() => {
            ...
            <div className="mt-[10px] mb-[30px]">
                <button onClick={onSubmitHandler} className="bg-primary-red h-[48px] w-full text-[#fff] text-[18px]">
                    다음
                </button>
            </div>
        </div>
