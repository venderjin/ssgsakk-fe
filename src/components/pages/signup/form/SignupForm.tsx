...
const schema = yup.object().shape({
    id: yup.string().required("아이디를 입력해주세요").matches(regex.id, "6~20자의 영문 소문자와 숫자만 사용 가능합니다"),
    password: yup.string().required("비밀번호를 입력해주세요").matches(regex.password, "8~20자의 영문, 숫자를 모두 포함해주세요"),
    passwordConfirm: yup
        .string()
        .oneOf([yup.ref("password"), ""], "비밀번호가 일치하지 않습니다")
        .required("비밀번호 확인을 입력해주세요"),
    name: yup.string().required("이름을 입력해주세요").matches(regex.name, "올바른 이름을 입력해주세요"),
    phone: yup.string().required("휴대폰 번호를 입력해주세요").matches(regex.phone, "'-' 제외 11자리를 입력해주세요"),
    email: yup.string().required(),
    zipCode: yup.string().required("주소를 입력해주세요"),
});

const SignupFormComponent = ({ userEmail, oauthId }: { userEmail: string; oauthId: string }) => {
                    ...
                    {openAddress ? (
                        ...
                        <>
                            ...
                            <form className=" text-[#666]" onSubmit={handleSubmit(onSubmit)}>
                                <div className="px-[20px]">
                                    ...
                                    <div className="py-[15px] flex border-t border-t-[#eee]">
                                        ...
                                        <div className="w-full">
                                            ...
                                            />
                                            {errors.passwordConfirm && <ErrorMessage error={errors.passwordConfirm.message} />}
                                        </div>
                                    </div>

                                    {/* 이름 */}
                                    <TextField text="이름" name="name" register={register} errorMsg={errors.name?.message} />
                                    ...
                                    <div className="py-[15px] flex border-t border-t-[#eee]">
                                        ...
                                        <div className="w-full relative">
                                            ...
                                            {errors.zipCode && <ErrorMessage error={errors.zipCode.message} />}
                                        </div>
                                    </div>
                                    ...
                                    {/* 휴대폰 번호 */}
                                    <TextField text="휴대폰번호" name="phone" register={register} errorMsg={errors.phone?.message} />
                                ...
                                <MarketingAgreeList onChangeMarketingAgreement={onChangeMarketingAgreement} />
                                <div className="mt-[10px] mb-[30px] px-[20px]">
                                    <button type="submit" className="w-full bg-primary-red text-[#fff] text-[18px] h-[48px]">
                                        가입하기
                                    </button>
                                </div>
                            </form>
