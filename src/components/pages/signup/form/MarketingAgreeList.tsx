...
const MarketingAgreeList = ({
  onChangeMarketingAgreement,
}: {
  onChangeMarketingAgreement: (checkList: CheckBoxType) => void;
}) => {
  ...
  return (
    <div className="px-[20px] pt-[15px]">
      <div className="pl-[10px]">
        {marketingAgreementData.map(
          (bigTerm: MarketingAgreementType, index) => (
            <div
              className="pb-[10px] mt-[10px] border-b border-b-[#eee]"
              key={index}
            >
                ...
                {bigTerm.terms.map((term) => (
                  <li className="my-[10px]" key={term.id}>
                    <div className="flex content-center items-top">
                      ...
                      <label
                        className="w-[80%]  text-[14px] text-[#222] mr-[20px]"
                        htmlFor={term.type}
                      >
              ...
              <div className="pt-[5px]  pl-[17px] text-[13px] text-primary-red ">
                마케팅 정보 수신 동의를 하시면 {bigTerm.title} 서비스 및 이벤트
                정보를 받으실 수 있습니다.
              </div>
            </div>
          )
      ...
      <div className="py-[15px]">
        <p className="text-[13px] text-[#666] font-bold ">
