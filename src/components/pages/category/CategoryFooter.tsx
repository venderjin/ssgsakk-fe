        ...
        <div className="bg-[#555] pt-[30px] pb-[170px] text-center flex justify-center">
            {session
                ? footerLoginData.map((data, index) => (
                      <div key={data.id} className={`pr-[10px] font-Pretendard text-[12px] text-[#cccccc] align-middle ${!index ? "" : "vetical-line"} `}>
                          {data.title === "로그아웃" ? (
                              <button
                                  onClick={() => {
                                      signOut();
                ...
                : footerData.map((data, index) => (
                      <Link key={data.id} href={data.url}>
                          <span className={`pr-[10px] font-Pretendard text-[12px] text-[#cccccc] align-middle ${!index ? "" : "vetical-line"} `}>
                              {data.title}
                          </span>
                      </Link>
                  ))}
