...
const EventProductList = async ({
  ...
  return (
    <div className="bg-white mt-5">
      {eventProductList
        .slice(0, visiableEventCount)
        .map((item: EventProductList, idx: number) => {
          ...
          return (
            <div key={idx} className="px-3 mb-3">
              <Link
                href={`/productList/eventProductList/${item.eventSeq}`}
                key={idx}
              >
                <div
                  style={{
                ...
                <div className="flex flex-row">
                  <div className="flex-none my-2 px-1 w-4/5 h-[80px]">
                    <div className="flex flex-row gap-1">
                      ...
                      <p className="font-Pretendard text-[16px]">
                        {item.eventName}
                      </p>
                    </div>
                    <p className="font-Pretendard text-[16px] font-bold">
                      {item.eventLowestPrice.toLocaleString()}원 ~
                    </p>
                    <p className="font-Pretendard text-[11px] text-gray-500 whitespace-nowarp">
                      종료 : {formattedDate}
                    </p>
                  </div>
