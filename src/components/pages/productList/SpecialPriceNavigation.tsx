...
const SpecialPriceNavigation = () => {
  const [resetSpecialPriceType, setResetSpecialPriceType] =
    useState<string>(""); //
  const [clickedCategoryIndex, setClickedCategoryIndex] = useState<number>(0); // 클릭한 버튼의 인덱스를 저장하는 상태
  const [clickedDeliveryTypeIndex, setClickedDeliveryTypeIndex] = useState<
    number | null
  >(null); // 클릭한 배송타입의 인덱스를 저장하는 상태
    ...
    <>
            ...
            item.deliveryType?.map((deliveryType, index) => (
              ...
              >
                <Image
                  ...
                  src={
                    clickedDeliveryTypeIndex === index
                      ? deliveryType.checkedImgpath
                      : deliveryType.unCheckedImgpath
                  }
      ...
      {isModalOpen && (
        <div className="mx-auto my-16 max-w-md">
          ...
          <button
            onClick={() => ModalHandler()}
            className="hover: animate-fade-up animate-once animate-ease-in"
          >
            close btn
          </button>
        </div>
