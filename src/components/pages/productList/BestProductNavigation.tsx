...
const BestProductNavigation = ({ deliveryType, categorySeq }: BestProductNavigationProps) => {
    ...
    useEffect(() => {
        if (activeDeliveryRef.current) {
            activeDeliveryRef.current.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" });
        }
            ...
            <div className="py-2 bg-white">
                <div className="flex flex-row h-[40px] items-center overflow-x-auto px-3">
                    {deliveryData.map((data) => (
                        <div
                            ref={selectedDelivery === data.deliveryType ? (activeDeliveryRef as React.RefObject<HTMLDivElement>) : null}
                            key={data.id}
                            onClick={() => {
                        ...
                        >
                            <Image
                                src={selectedDelivery === data.deliveryType ? data.checkedImgpath : data.unCheckedImgpath}
                    ...
                    <div className="flex flex-row h-[40px] items-center overflow-x-auto px-3">
                        {categoryData.map((data) => (
                            <button
                                ...
                                ${selectedCategory === data.id ? "bg-[#222222] text-white" : "bg-white text-black border-2 border-[#E5E5E5]"}
                                `}
                            >
                                {data.title}
                            </button>
                ...
                <div className="flex flex-col gap-2 p-3">
                    {categoryData.map((item) => (
                        <label key={item.id} className={`mx-3 py-2 flex justify-start items-center ${selectedCategory === item.id ? "font-bold" : ""}`}>
                            ...
                            />
                            <span className="text-black font-Pretendard text-[16px]">{item.title}</span>
                        </label>
                    ))}
                </div>
            </SliderModal>
