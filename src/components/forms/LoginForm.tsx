...
export default function LoginForm({ callbackUrl, error }: { callbackUrl: string | null; error: string | null }) {
    ...
    const loginSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        //로그인 버튼을 누를 때 쿠키에 저장
        if (saveIdCheck) setCookie("rememberUserId", loginData.loginId, { path: "/" });
        ...
        const response = await signIn("user-credentials", {
            ...
            //로그인 실패 시 새로고침 여부
            redirect: true,
            callbackUrl: callbackUrl && callbackUrl !== "undefined" ? `${callbackUrl}` : "/mypage",
        });
    };
    ...
    return (
        <div className="p-[20px] pt-[40px]">
            {error && error !== "undefined" && <Notify message="아이디 혹은 패스워드가 틀립니다." />}
            <form onSubmit={loginSubmit}>
                ...
                <div className="flex w-full mt-[10px] px-[15px]">
                    <LoginCheckbox id="keep_id" isChecked={saveIdCheck} labelName="아이디 저장" onChangeCheckbox={onInputCheckboxChange} iconSize={22} />
                </div>

                <button type="submit" className="base-btn bg-[#FF5452] text-white mt-[30px] font-medium">
                    로그인
                </button>
            </form>
            ...
            <ul className="w-full my-[40px] flex justify-around items-center">
                {snsLoginData.map((sns: SnsLogin) => {
                    return <SnsButton key={sns.id} snsType={sns.type} snsName={sns.name} iconPosition={sns.img} />;
                })}
            </ul>

            <div className="mt-[100px]">
                <Link href="">
                    <button className="base-btn bg-[#222222] text-white text-[14px] font-semibold mb-[34px]">휴대폰 간편 로그인</button>
                </Link>

                <Link href="/nonMemberLogin">
                    <button className="base-btn bg-[#f3f3f3] text-[#9B9B9B] text-[13px]">비회원 조회하기</button>
                </Link>
            </div>
        </div>
    );
