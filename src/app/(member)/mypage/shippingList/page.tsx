...
const getShippingData = async (shippingAddressSeq: number[], token: string) => {
    const data = shippingAddressSeq.map(async (shippingAddressSeq: number) => {
        const res = await fetch(`${process.env.BASE_URL}/shipping-addr/${shippingAddressSeq}`, {
            cache: "no-cache",
            headers: {
                Authorization: token,
                "Content-Type": "application/json",
...
const ShippingList = async () => {
    ...
    const shippingAddressList = await fetchShippingList(session?.user?.token || "");
    const shippingData: ShippingInfoType[] = await getShippingData(shippingAddressList, session?.user?.token || "");
